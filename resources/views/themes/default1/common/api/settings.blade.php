...
{!! Form::open(['url'=>'api','method'=>'post','files'=>true]) !!}
    <div class="box-header with-border">
        <h3 class="box-title">{{trans('lang.api_settings')}}</h3>
    ...
    <div class="box-body">
        <div class="row">
            <div class="col-md-12">
                ...
                @endif
                        <h4>{!! trans('lang.api_configurations') !!}</h4>
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <br/>
                <a class="btn btn-primary" id="generate"> <i class="fa fa-refresh"> </i> {!! trans('lang.generate_key') !!}</a>
            </div>
        </div>
            </div>
        </div>
...
// Close a ticket
    $('#generate').on('click', function(e) {
        $.ajax({
            ...
            beforeSend: function() {
                $("#generate").empty();
                var message = "<i class='fa fa-refresh fa-spin'> </i>  <?php echo trans('lang.generate_key'); ?>";
            ...
            success: function(response) {
                ...
                $("#generate").empty();
                var message = "<i class='fa fa-refresh'> </i>  <?php echo trans('lang.generate_key'); ?>";
                $('#generate').html(message);
            }
        })
        return false;
