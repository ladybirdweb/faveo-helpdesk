...
class PhpMailController extends Controller {
    ...
    public function sendmail($from, $to, $message, $template_variables) {
        ...
        if ($template_type == 'ticket-reply-agent') {
            ...
            $contents = $line . $messagebody;
        } else {
            $contents = $messagebody;
        }
        $send = $this->laravelMail($recipants, $recipantname, $subject, $contents, $cc, $attachment);
        return $send;
    }
    ...
    public function sendEmail($from, $to, $message) {
        try {
            ...
            $subject = $this->checkElement('subject', $message);
            $content = $this->checkElement('body', $message);
            //$template = $this->checkElement('scenario', $message);
            $attachment = $this->checkElement('attachments', $message);
            return $this->laravelMail($recipants, $recipantname, $subject, $content, $cc, $attachment);
        } catch (Exception $e) {
            dd($e);
        }
    }
     ...
     */
    public function sendEmailold($from, $to, $message) {
        try {
            $from_address = $this->fetch_smtp_details($from);
            if ($from_address == null) {
                return $from_address;
    ...
    public function sendmailold($from, $to, $message, $template_variables) {
//        try {
        // dd($from);
        $from_address = $this->fetch_smtp_details($from);
        if ($from_address == null) {
