...
class AgentController extends Controller
{
    ...
    {
        try {
            return view('admin.helpdesk.agent.agents.index');
        } catch (Exception $e) {
    ...
    public function create(Timezones $timezone, Groups $group, Department $department, Teams $team_all, CountryCode $code)
    {
        try {
            ...
            return view('admin.helpdesk.agent.agents.create', compact('assign', 'teams', 'agents', 'timezones', 'groups', 'departments', 'team', 'send_otp'))->with('phonecode', $phonecode->phonecode);
        } catch (Exception $e) {
    ...
    public function edit($id, User $user, Assign_team_agent $team_assign_agent, Timezones $timezone, Groups $group, Department $department, Teams $team, CountryCode $code)
    {
        try {
            ...
            return view('admin.helpdesk.agent.agents.edit', compact('teams', 'assign', 'table', 'teams1', 'selectedTeams', 'user', 'timezones', 'groups', 'departments', 'team', 'exp', 'counted'))->with('phonecode', $phonecode->phonecode);
        } catch (Exception $e) {
