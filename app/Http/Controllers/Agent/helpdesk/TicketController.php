...
class TicketController extends Controller {
    ...
    public function newticket1(CountryCode $code, Request $request) {
        ...
        foreach ($assignto_agent as $agent) {
            ...
            $html1 .= "<option value='user_" . $agent->id . "'>" . $agent->first_name . "</option>";
        }
...
//                Event fire for new ticket['']
    ...
    public function createTicket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assigned_to, $team_id, $form_data, $status) {
        ...
        // dd($ticket->assigned_to,$ticket->team_id);
        ...
        if ($ticket->team_id != null) {
            ...
            //mail team lead and all member
            $UserEmail = $ticket->team_id;
            ...
            $ticket_subject = 'Assign ticket';
            $master = Auth::user()->first_name . ' ' . Auth::user()->last_name; //ticket assigner
...
//        $ticket_status = $ticket->where('id', '=', $id)->first();
    ...
    public function assign($id) {
        ...
        if ($assign_to[0] == 'team') {
            ...
            $member_ids = DB::table('team_assign_agent')->select('agent_id as user_id')->where('team_id', '=', $assign_to[1])->get();

            $ticket_thread = Ticket_Thread::where('ticket_id', '=', $id)->first();
            $ticket_subject = $ticket_thread->title; //ticket subject
            $master = Auth::user()->first_name . ' ' . Auth::user()->last_name; //ticket assigner
                        ...
                        ->addColumn('assigned_to', function ($ticket) {
                            if ($ticket->assigned_to == null && $ticket->team_id == null) {
                                ...
                                $assign = DB::table('teams')->where('id', '=', $ticket->team_id)->first();
                                 if(Auth::user()->role == 'admin'){
                                $url = route('teams.profile.show', $ticket->team_id);
                                // return "$assign->name";
