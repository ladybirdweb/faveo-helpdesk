...
if (Auth::user()->role == 'agent') {
    ...
    $tickets = App\Model\helpdesk\Ticket\Tickets::where('status','=',1)->where('follow_up','=','1')->where('dept_id', '=', $dept->id)->orderBy('id', 'DESC')->count();
} else {
    $tickets = App\Model\helpdesk\Ticket\Tickets::where('status','=',1)->where('follow_up','=','1')->orderBy('id', 'DESC')->count();
}
?>
<!-- Main content -->
<div class="box box-primary">
    <div class="box-header with-border">
        <h3 class="box-title">{!! Lang::get('lang.followup_tickets') !!} </h3> <small id="title_refresh">{!! $tickets !!} {!! Lang::get('lang.tickets') !!}</small>
    </div><!-- /.box-header -->
    ...
    <div class="box-body ">
        ...
        @endif
         @include('themes.default1.agent.helpdesk.filters.filter')
