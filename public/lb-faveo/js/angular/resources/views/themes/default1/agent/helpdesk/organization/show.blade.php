...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="box box-widget widget-user">
            ...
            <div class="box-footer no-padding">
                ...
                <button data-toggle="modal" data-target="#assign_head" id="button_select" class="btn btn-primary">{!! Lang::get('lang.select_manager') !!}</button>
                 @if($userss!=0)
                 <button data-toggle="modal" data-target="#assign_head1" id="button_select1" class="btn btn-danger pull-right">{!! Lang::get('lang.edit_manager') !!}</button>
                 @endif
            </div>
            </div>
        </div>
    ...
    <div class="col-md-8">
                ...
                <table class="table table-hover table-bordered">
                    <tbody><tr>
                        ...
                        <tr>
                            ...
                            <td><a href="{!! route('user.show',$user_detail->id) !!}">{!! $user_detail->first_name.' '.$user_detail->last_name !!}</a></td>
                            @else
                            <td><a href="{!! route('user.show',$user_detail->id) !!}">{!! $user_detail->user_name !!}</a></td>
                            @endif                            <td><a href="{!! route('user.show',$user_detail->id) !!}">{!! $user_detail->email !!}</a></td>
                    ...
                    @endif
                            ...
                            <td><a href="{!! route('user.show',$user_detail->id) !!}" class="btn btn-primary btn-xs">{!! Lang::get('lang.view')!!}</a></td>
        ...
        if (!$user_orga_relations->isEmpty()) {
...
//        $models = \App\Model\helpdesk\Ticket\Tickets::whereIn('user_id', $user_orga_relation_id)->get();
       ...
       // dd($open);
        ...
        <?php  \Event::fire('orgdocs.upload', array($org_id=$orgs->id)); ?> 
...
<script type="text/javascript">
            ...
            $('.tagit-plgn').tagit({
            tagSource: "{{route('post.org-head.autofill', $orgs->id)}}",
            allowNewTags: false,
